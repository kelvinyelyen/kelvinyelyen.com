---
title: "Iterating Toward Realism"
...
**Abstract:** *This article explores how different mathematical inputs (deterministic sinusoids, Gaussian noise, and Poisson processes) affect the Leaky Integrate-and-Fire (LIF) model. By transitioning from continuous-time differential equations to discrete-time Euler integration, we demonstrate how stochastic inputs bring computational models significantly closer to biological realism compared to idealized constant currents.*
...
$$
V(t+\Delta t) = V(t) + \frac{\Delta t}{\tau_m}\Big(E_L - V(t) + R\,I(t)\Big)
$$
...
After each Euler update, if $V(t+\Delta t) \ge V_{\text{th}}$, record spike and reset:

$$
V(t+\Delta t) \leftarrow V_{\text{reset}}
$$
...
V(0) &= -65~\text{mV} \\
V_{\text{th}} &= -50~\text{mV}, \quad V_{\text{reset}} = -65~\text{mV}
\end{aligned}
$$

Note: $R \cdot I$ gives units in mV, e.g., $R \cdot 1$ nA = 10 mV.
...
| $t$ (ms) | $V_{\text{const}}$ | $V_{\text{sin}}$ | $I_{\text{sin}}$ | $V_{\text{noise}}$ | $I_{\text{noise}}$ | $V_{\text{pois}}$ | spike |
|----------|-------------------|------------------|------------------|--------------------|-------------------|------------------|-------|
...
Below is a live simulation of the Leaky Integrate-and-Fire neuron. You can adjust the parameters and input currents to observe how the membrane potential integrates, leaks, spikes, and resets in real time.
...
- [Leaky Integrate-and-Fire (LIF) Neuron Simulation](https://colab.research.google.com/drive/1ecaifJtgvZBc8QzcabNCsXnObv0ssqGj?usp=sharing)
- [Interactive Simulation](https://and-simulator.vercel.app/labs/lif)